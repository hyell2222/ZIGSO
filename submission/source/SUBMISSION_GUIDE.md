...
`npm run export` runs a **static** build (submission용 `dist/`). This is the same step as before; it does not use the Vercel/server build.

```bash
npm run export
```

Copy all files from `dist/` into `submission/program/`.
