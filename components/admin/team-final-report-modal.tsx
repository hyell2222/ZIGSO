...
/** 학생 `/play` 최종 보고서 제출 폼과 동일한 뼈대·다크 톤, 읽기 전용. */
export function TeamFinalReportModal({ isOpen, onClose, team, teamDisplayName, suspectRoster }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="team-report-modal-title"
    >
      <button
        type="button"
        className="absolute inset-0 bg-[var(--ink-88)] backdrop-blur-[2px]"
        aria-label="닫기"
        onClick={onClose}
      />
      <div
        className={cn(
          "relative z-10 flex max-h-[min(92dvh,880px)] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-[color-mix(in_srgb,var(--primary)_26%,transparent)]",
          "bg-[color-mix(in_srgb,var(--mystery)_78%,var(--ink))] text-[color:var(--entry-parchment)]",
          "shadow-[0_24px_60px_color-mix(in_srgb,var(--ink)_50%,transparent)]",
        )}
      >
        <div className="flex items-center justify-between border-b border-[color-mix(in_srgb,var(--primary)_18%,transparent)] bg-[color-mix(in_srgb,var(--mystery)_55%,#151210)] px-4 py-3 pl-5">
          <p id="team-report-modal-title" className="text-sm font-semibold text-[color:var(--entry-parchment)]">
            {teamDisplayName} — 최종 보고서
          </p>
          <button
            type="button"
            onClick={onClose}
            className="rounded-md p-2 text-[color:var(--entry-parchment-muted)] transition hover:bg-[color-mix(in_srgb,var(--entry-parchment)_8%,transparent)] hover:text-[color:var(--entry-parchment)]"
            aria-label="모달 닫기"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
          {!team ? (
            ...
            <p className="text-sm text-[color:var(--entry-parchment-muted)]">이 팀은 아직 최종 보고를 제출하지 않았습니다.</p>
          ) : (
            <Card className="overflow-hidden border-[color-mix(in_srgb,var(--primary)_26%,transparent)] bg-[color-mix(in_srgb,var(--mystery)_72%,var(--ink))] text-[color:var(--entry-parchment)] shadow-none">
              ...
              <CardContent className="space-y-5 px-4 pb-5 pt-5 sm:px-5">
                <div className="rounded-md border border-[color-mix(in_srgb,var(--primary)_40%,transparent)] bg-[color-mix(in_srgb,var(--primary)_14%,#141a17)] px-3 py-2.5 text-sm text-[color:var(--entry-parchment)] shadow-[0_0_20px_color-mix(in_srgb,var(--primary)_8%,transparent)]">
                  ...
                  <p className="mt-0.5 font-medium">팀 보고서가 접수되었습니다.</p>
