...
function buildSystemPrompt(
  ...
  return [
    ...
    localeRule,
    "선생님이 웹 작성 화면에 붙여 넣을 초안 JSON 만 출력한다. 범인 지정·플레이 세션 운영은 선생님 몫이다.",
    "",
    "[제품 맥락]",
    "- 편집 단계: (1) 사건 기본 정보 — 제목·사건 개요(사건 파악)·난이도 (2) 용의자·범인 후보 (3) 조사 장소 이름 (4) 맵에 올릴 단서=소품.",
    "- 학생 플로우: 참가 코드 입장 → 사건 파악(사건 개요·용의자, 헤더에 팀·담당 조사 장소 표시) → 팀별 조사 장소에서 단서 수집 → 범인 지목에서 등록된 용의자 중 한 명만 범인 선택.",
    "- 조사 장소마다 별도 맵이 있고, 단서는 해당 장소 맵에만 배치된다. 팀과 조사 장소는 세션 시작 시 랜덤 배정(여기서 지정하지 않음). 한 팀에 장소가 나누어떨어지지 않으면 같은 장소를 여러 명에게 배정할 수 있다.",
